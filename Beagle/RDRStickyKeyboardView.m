...
- (UIButton *)rightButton
{
    ...
    [_rightButton setTitle:NSLocalizedString(@"Post", nil)
                  forState:UIControlStateNormal];
    [_rightButton addTarget:self action:@selector(postButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)postButtonClicked:(id)sender{
    [delegate updateInterestedStatus];
...
@interface RDRStickyKeyboardView () <UITextViewDelegate,RDRKeyboardInputViewDelagte> {
...
//    self.dummyInputView.delegate=self;
...
- (void)_scrollViewAdaptInsetsToKeyboardFrame:(CGRect)keyboardFrame
{
    ...
    CGRect scrollViewFrame = self.scrollView.frame;
    scrollViewFrame.origin.y=self.frame.size.height-keyboardHeight-44-64+20;
    //    scrollViewFrame.size.height = self.scrollView.frame.size.height - self.inputView.frame.size.height;
    self.scrollView.frame = scrollViewFrame;
...
- (BOOL)textViewShouldEndEditing:(UITextView *)textView
{
    ...
    if(dismissKey){
        ...
        CGRect scrollViewFrame =  self.scrollView.frame;
        scrollViewFrame.origin.y=278.845;
        
        scrollViewFrame.size.height=253.155;
        self.scrollView.frame = scrollViewFrame;
    }
