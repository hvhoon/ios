...
@interface EventBlurView ()<UIGestureRecognizerDelegate,TimeFilterDelegate,CustomPickerViewDelegate,UIScrollViewDelegate>
...
+ (EventBlurView *) load:(UIView *) view {
    EventBlurView *blur = [[[NSBundle mainBundle] loadNibNamed:@"EventBlurView" owner:nil options:nil] objectAtIndex:0];
...
- (void) crossDissolveHide {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(dismissactivityTimeFilter)])
        [self.delegate dismissactivityTimeFilter];
